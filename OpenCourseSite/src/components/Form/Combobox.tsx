...
const fetchCourseOptions = async (selectedTerm: string, query: string): Promise<Course[]> => {
    const response = await fetch(`http://localhost:8081/coursesearch/${selectedTerm}/${query}`);
    if (!response.ok) {
